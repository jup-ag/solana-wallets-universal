...
export class HardcodedWalletStandardAdapter extends BaseSignerWalletAdapter {
  name = "" as WalletName
  url = ""
  icon = ""
  deepUrl?: (loc: Location) => string
  ...
  async signTransaction<T extends any>(_: T): Promise<any> {}
}

export const SolanaMobileWalletAdapterWalletName = "Mobile Wallet Adapter"

export const HARDCODED_WALLET_STANDARDS: THardcodedWalletStandardAdapter[] = [
  {
    id: "Phantom",
    ...
    deepUrl: loc => {
      ...
      const ref = encodeURIComponent(loc.origin)
  ...
  },
  {
    ...
    deepUrl: loc => {
      const url = encodeURIComponent(loc.href)
      return `okx://wallet/dapp/url?dappUrl=${url}`
    },
  },
]
